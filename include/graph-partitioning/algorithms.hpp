...
public:
  RemoveUnsupportedNodes(const std::unordered_set<std::string> &ops,
      ...
      : ops_{ops}, data_ops_{data_ops}, id_ops_{id_ops} {};

  void operator()(const graph::Graph &g, graph::Graph &g_out,
...
std::map<graph::Edge, graph::Edge>
summarize_subgraphs(const graph::Graph &g_orig, graph::Graph &g,
                    std::map<int, graph::Subgraph> &subgraphs,
                    EType &orig_etype, EName &orig_ename, VCompat &orig_vcompat,

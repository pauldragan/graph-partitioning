...
namespace boost {
enum vertex_node_type_t { vertex_node_type };

enum vertex_compat_t { vertex_compat };

enum vertex_subgraph_index_t { vertex_subgraph_index };
...
namespace partition {
namespace graph {
enum class Compat { UNKNOWN, COMPATIBLE, INCOMPATIBLE, BOUNDARY, DATA };

enum class ETypes { UNKNOWN, C2C, I2I, C2I, I2C };
...
using EdgeProperty =
    boost::property<edge_type_t, ETypes,
...
using VertexNameProperty =
    boost::property_map<graph::Graph, graph::vertex_name_t>::type;
using VertexCompatProperty =
    boost::property_map<graph::Graph, graph::vertex_compat_t>::type;
using VertexTypeProperty =
    boost::property_map<graph::Graph, graph::vertex_node_type_t>::type;
using VertexSubgraphIndexProperty =
    boost::property_map<graph::Graph, graph::vertex_subgraph_index_t>::type;
using VertexIndexProperty =
    boost::property_map<graph::Graph, graph::vertex_index_t>::type;
using EdgeTypeProperty =
    boost::property_map<graph::Graph, graph::edge_type_t>::type;
using EdgeIndexProperty =
    boost::property_map<graph::Graph, graph::edge_index_t>::type;
using EdgeNameProperty =
    boost::property_map<graph::Graph, graph::edge_name_t>::type;

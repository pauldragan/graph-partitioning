...
void ONNXWriter::to_bin(std::string orig_path,
			std::string out_dir,
			graph::Graph& g_partitioned,
			std::map<int, graph::Subgraph>& subgraphs) {
  GOOGLE_PROTOBUF_VERIFY_VERSION;

  std::fstream modelpb (orig_path,
			std::ios::in |
			std::ios::binary);

  onnx::ModelProto model;
  if (!model.ParseFromIstream(&modelpb)) {
    std::cerr << "[ERROR] ONNXWriter: failed to parse from stream!" << std::endl;
    return;
  }
  const onnx::GraphProto& orig_graph = model.graph();

  graph::VertexNameProperty vnames = boost::get(graph::vertex_name, g_partitioned);
  graph::VertexSubgraphIndexProperty vsubgraphs = boost::get(graph::vertex_subgraph_index, g_partitioned);
  ...
  for (auto it = orig_graph.value_info().begin(); it != orig_graph.value_info().end(); it++) {
    named_values[it->name()] = *it;
  }
  for (auto it = orig_graph.input().begin(); it != orig_graph.input().end(); it++) {
    named_inputs[it->name()] = *it;
    named_values[it->name()] = *it;
  }
  for (auto it = orig_graph.output().begin(); it != orig_graph.output().end(); it++) {
    named_outputs[it->name()] = *it;
    named_values[it->name()] = *it;
  }
  for (auto it = orig_graph.initializer().begin(); it != orig_graph.initializer().end(); it++) {
  ...
  int i = 0;
  for (auto it = orig_graph.node().begin(); it < orig_graph.node().end(); it++) {
  ...
  for (auto it = subgraphs.begin(); it != subgraphs.end(); it++) {
    ...
    for (auto it2 = it->second.input_edges.begin(); it2 != it->second.input_edges.end(); it2++) {
      graph::Edge e = *it2;
      std::string en = enames[e];
      subgraph_inputs[it->first].insert(en);
    }

    for (auto it2 = subgraph_inputs[it->first].begin(); it2 != subgraph_inputs[it->first].end(); it2++) {
  ...
  for (auto it = subgraphs.begin(); it != subgraphs.end(); it++) {
    ...
    *subgraph_model.mutable_opset_import() = {model.mutable_opset_import()->begin(),
					      model.mutable_opset_import()->end()};
    *subgraph_model.mutable_metadata_props() = {model.mutable_metadata_props()->begin(),
						model.mutable_metadata_props()->end()};
    *subgraph_model.mutable_training_info() = {model.mutable_training_info()->begin(),
    ...
    for (auto it2 = subgraph_nodes[it->first].begin(); it2 != subgraph_nodes[it->first].end(); it2++) {
    ...
    for (auto it2 = subgraph_outputs[it->first].begin(); it2 != subgraph_outputs[it->first].end(); it2++) {
    ...
    for (auto it2 = subgraph_initializers[it->first].begin(); it2 != subgraph_initializers[it->first].end(); it2++) {
  ...
  for (auto it = onnx_models.begin(); it != onnx_models.end(); it++) {
    ...
    file_name << out_dir << "/" << "subgraph_" << it->first << ".onnx";
    std::fstream subgraphpb (file_name.str(),
			     std::ios::out |
			     std::ios::binary);

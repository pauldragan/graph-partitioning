...
void RemoveUnsupportedNodes::operator()(
    const graph::Graph &g, graph::Graph &g_out,
    std::map<int, graph::Subgraph> &subgraphs_out) {
  ...
  auto v_compat =
      compat_visitor(ops_, ntype, compat, compat_nodes, incompat_nodes);

  auto e_compat = edge_compat_visitor(compat, etype, c2c_edges, c2i_edges,
                                      i2c_edges, i2i_edges);
  ...
  auto data_initialize =
      data_visitor_initialize(data_ops_, ntype, compat, data_nodes);
  boost::depth_first_search(
      G, boost::visitor(boost::make_dfs_visitor(data_initialize)));

  std::vector<graph::Vertex> id_nodes;
  auto classify_identity =
      classify_identity_visitor(id_ops_, ntype, compat, id_nodes);
  boost::depth_first_search(
      G, boost::visitor(boost::make_dfs_visitor(classify_identity)));

  auto relabel_visitor = relabel_data_visitor(ntype, compat);
  boost::depth_first_search(
      G, boost::visitor(boost::make_dfs_visitor(relabel_visitor)));

  boost::depth_first_search(G,
                            boost::visitor(boost::make_dfs_visitor(e_compat)));

  std::map<int, std::vector<graph::Vertex>> subgraph_nodes;
  auto subgraph_initialize =
      subgraph_visitor_initialize(subgraph_nodes, compat, subgraph);
  auto subgraph_edges =
      subgraph_visitor_edges(subgraph_nodes, compat, subgraph);
  boost::depth_first_search(
      G, boost::visitor(boost::make_dfs_visitor(
             boost::make_list(subgraph_initialize, subgraph_edges))));
  ...
  boost::depth_first_search(
      G, boost::visitor(boost::make_dfs_visitor(assign_edges)));
  ...
  for (auto it = subgraph_map.begin(); it != subgraph_map.end(); it++) {
    ...
    for (auto it2 = subg.output_edges.begin(); it2 != subg.output_edges.end();
         it2++) {
  ...
  do {
    ...
    std::map<graph::Edge, graph::Edge> edge_map =
        algorithms::summarize_subgraphs(G, G_summary, subgraph_map, etype,
                                        ename, compat, subgraph);
    graph::VertexSubgraphIndexProperty sum_subgraph =
    ...
    boost::depth_first_search(
        G_summary, boost::visitor(boost::make_dfs_visitor(cycles_visitor)));
    ...
    if (cycles.size() > 0) {
      ...
      auto old_location =
          std::find(old_subgraph.nodes.begin(), old_subgraph.nodes.end(), v);

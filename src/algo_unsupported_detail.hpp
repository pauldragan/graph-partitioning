...
namespace partition::algorithms::detail {
  ...
  struct assign_edges_subgraph_visitor : public boost::base_visitor<assign_edges_subgraph_visitor<CompatProp, SubgraphProp>> {
  ...
  struct relabel_data_visitor : public boost::base_visitor<relabel_data_visitor<TypeProp, CompatProp>> {
    relabel_data_visitor(const TypeProp& type_prop,
			      CompatProp& compat_prop) :
	...
	return;
      ...
      bool same_type = std::all_of(target_types.begin(),
      				   target_types.end(),
  ...
  template <typename TypeProp, typename CompatProp>
  struct classify_identity_visitor : public boost::base_visitor<classify_identity_visitor<TypeProp, CompatProp>> {
    classify_identity_visitor(const std::unordered_set<std::string>& ops,
			      const TypeProp& type_prop,
			      CompatProp& compat_prop,
  ...
  template <typename TypeProp, typename CompatProp>
  struct data_visitor_initialize : public boost::base_visitor<data_visitor_initialize<TypeProp, CompatProp>> {
  ...
  template <typename TypeProp, typename CompatProp>
  struct compat_visitor : public boost::base_visitor<compat_visitor<TypeProp, CompatProp>> {
    ...
    compat_visitor(const std::unordered_set<std::string>& ops,
      ...
      type_prop_{type_prop},
      compat_prop_{compat_prop},
      compat_nodes_{compat_nodes},
      incompat_nodes_{incompat_nodes} {}
  ...
  struct edge_compat_visitor : public boost::base_visitor<edge_compat_visitor<VCompatProp, ECompatProp>> {

    edge_compat_visitor(VCompatProp& v_compat_prop,
			ECompatProp& e_compat_prop,
      ...
      v_compat_prop_{v_compat_prop},
      e_compat_prop_{e_compat_prop},
      c2c_edges_{c2c_edges},
      c2i_edges_{c2i_edges},
      i2c_edges_{i2c_edges},
  ...
  template <typename CompatProp, typename SubgraphProp>
  struct subgraph_visitor_edges : public boost::base_visitor<subgraph_visitor_edges<CompatProp, SubgraphProp>> {
    subgraph_visitor_edges(std::map<int, std::vector<graph::Vertex>>& subgraph_map,
			   const CompatProp& compat_prop,
			   SubgraphProp& subgraph_prop) :
      subgraph_map_{subgraph_map},
      compat_prop_{compat_prop},
  ...
  template <typename CompatProp, typename SubgraphProp>
  struct subgraph_visitor_initialize : public boost::base_visitor<subgraph_visitor_initialize<CompatProp, SubgraphProp>> {
    subgraph_visitor_initialize(std::map<int, std::vector<graph::Vertex>>& subgraph_map,
				const CompatProp& compat_prop,
				SubgraphProp& subgraph_prop
				) :
      subgraph_map_{subgraph_map},
  ...
  struct record_predecessors : public boost::base_visitor<record_predecessors> {
    record_predecessors(std::map<graph::Vertex, std::vector<graph::Vertex>>& predecessors) :
  ...
  struct solve_cross_edges : public boost::base_visitor<solve_cross_edges<Subgraph>> {
    solve_cross_edges(std::map<graph::Vertex, std::vector<graph::Vertex>>& predecessors,
	...
	std::set_intersection(preds_u.begin(), preds_u.end(),
			      preds_v.begin(), preds_v.end(),
	...
	std::cout << "moving " << v << " to subgraph " << current_size << std::endl;
	...
	std::cout << "removing " << v << " from subgraph " << old_subgraph_idx << std::endl;
	auto& old_subgraph = subgraphs_[old_subgraph_idx];
	old_subgraph.erase(std::remove(old_subgraph.begin(), old_subgraph.end(), v));
  ...
  void merge_subgraphs(int A_idx, graph::Subgraph& A, int B_idx, graph::Subgraph& B, VSubgraph subgraph_prop) {
    ...
    std::move(A.output_edges.begin(), A.output_edges.end(), std::back_inserter(B.interior_edges));
    std::move(A.interior_edges.begin(), A.interior_edges.end(), std::back_inserter(B.interior_edges));
  }

  struct cycles_visitor : public boost::base_visitor<cycles_visitor> {
    cycles_visitor(bool* out) : out_{out} {
      *out_ = false;
    }
  ...
  template <typename Subgraph>
  struct record_cycles_visitor : public boost::base_visitor<record_cycles_visitor<Subgraph>> {
    record_cycles_visitor(std::vector<graph::Edge>& out, Subgraph& subgraph_prop) : out_{out}, subgraph_prop_{subgraph_prop} {}
    ...
    void operator()(graph::Edge e, const graph::Graph& g) {
      graph::Vertex u = boost::source(e, g);
      graph::Vertex v = boost::target(e, g);
      std::cout << "Cycle found : " << e << " subgraphs " << subgraph_prop_[u] << " -> " << subgraph_prop_[v] << std::endl;
      out_.push_back(e);
    }
